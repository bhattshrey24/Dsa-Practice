...
public class HashMap_Implementation<K, V> {// We are using 'Generic' concept
	...
	public HashMap_Implementation() {
		...
		initBuckets(4);// we just initializing 4 buckets
	...
	private void initBuckets(int N) {// simply initializing the buckets with empty linkedList
		...
		for (int bi = 0; bi < buckets.length; bi++) {
			buckets[bi] = new LinkedList<HashMap_Implementation<K, V>.HMNode>(); // we have to initialize with empty
	...
	// then we simply linearly check in that buucket only that wehter the key is
	...
	// arrayList of size twice the previous one and copy all the elements in it by
	// calling put.
	...
	private void reHash() {
		...
		for (int i = 0; i < oldBuckets.length; i++) {
			for (HMNode node : oldBuckets[i]) {
				put(node.key, node.value); // simply put it in new buckets
			}
		}
	}
	...
	private int getIndexWithinBucket(K key, int bi) {
		int idx = 0;
		for (HMNode hm : buckets[bi]) {

			if (hm.key.equals(key)) { // comparing the keys if they are same or not
				return idx;
			}
			idx++;
		}
