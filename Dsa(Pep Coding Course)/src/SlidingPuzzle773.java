...
public class SlidingPuzzle773 {
	...
	public static int slidingPuzzle(int[][] board) {
        // Step 1 make arraylist of possible positions where 0 can go without going out of bound if it is currently at position i  
		ArrayList<Integer>[] ar = new ArrayList[6];
		// possible places 0 can move if its at position ar[i]
		for (int i = 0; i < ar.length; i++) {
			ar[i] = new ArrayList<>(); // just initializing array list basically putting an empty arraylist at every index
		}
		ar[0].add(1);
		ar[0].add(3);
		ar[1].add(0);
		...
		//Step 2 converting given board into linear string using concept of converting 2d matrix to linear matrix using cell number
		String givenBoard = "";
		int initialPosOf0 = 0;
		...
		while (!que.isEmpty()) {
			...
			for (int pponz : ar[currEle.posOfZero]) { // ppoz=possible position of next 0
				...
				if (pponz < currEle.posOfZero) { // this is the case when current position of 0 > next position of 0
					...
					// char a=(char)Integer.toString(curr.).charAt(0);
					b = currEle.permu.charAt(currEle.posOfZero);// i.e 0
					st = currEle.permu.substring(0, pponz) + b + currEle.permu.substring(pponz + 1, currEle.posOfZero)
							+ a + currEle.permu.substring(currEle.posOfZero + 1);
