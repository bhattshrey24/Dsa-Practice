...
//import WeightedUndirectedGrapg.Edge;
//OPTIMIZE TC IS VERY BAD
...
//		pair.add(dum2);
	...
	public static String smallestStringWithSwaps(String s, List<List<Integer>> pairs) {
		...
		String solStr = s;
		WeightedUndirectedGraphHM ob = new WeightedUndirectedGraphHM();
		...
		for (int i = 0; i < s.length(); i++) {
			if (!visited[i] && ob.grf.containsKey(i)) {// the 2nd one i.e. ob.grf.containsKey(i) is for those which
														// cannot be swapped with anyone so basically they are not in
	...
	public static void dfsRecursion(HashMap<Integer, List<Integer>> gf, boolean[] visited, int vertx,
		...
		for (int nbr : gf.get(vertx)) {
			if (!visited[nbr]) {// if neighbor not visited then do dfs
				dfsRecursion(gf, visited, nbr, characters, indx, givenStr); // adding neighbors
			}
		}
	}
